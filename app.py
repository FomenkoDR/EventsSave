...
def get_files():
    ...
    #3
    res_2 = s.post('https://adfs.vsk.ru/adfs/ls/', data = {'SAMLRequest': SAMLRequest,'RelayState': RelayState}, timeout=None)

    client_request_id = BeautifulSoup(res_2.text, 'lxml').find('div', class_="groupMargin").find('form').get('action')

    #4

    res_3 = s.post('{}{}'.format('https://adfs.vsk.ru/adfs/ls/',client_request_id) , data={'UserName': 'DFomenko@vsk.ru' ,'Password': '22119912051847fdR','AuthMethod': 'FormsAuthentication'}, timeout=None)
    ...
    #5
    res_4 = s.post('https://authd.vsk.ru/auth/realms/users_auth/broker/adfs/endpoint', data={'SAMLResponse': SAMLResponse,'RelayState': RelayState_2}, allow_redirects=False, timeout=None)
